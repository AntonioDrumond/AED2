...
public class Q11{
	...
	static void sortGen(Pokemon[] arr, TP tp){
		int n = arr.length;
		int maior = arr[0].getGeneration();
		for(int i=0; i<n; i++){ // obter o maior
			int x = arr[i].getGeneration();
			if (x > maior) maior = x;
		}
		...
		for(int i=0; i<n; count[arr[i].getGeneration()]++, i++);

		for(int i=1; i<count.length; count[i] += count[i-1], i++);

		for(int i=n-1; i>=0; i--){
			ordenado[count[arr[i].getGeneration()]-1] = arr[i];
			count[arr[i].getGeneration()]--;
			tp.move();
		}
	...
	static void sortId(Pokemon[] arr, TP tp){
		int n = arr.length;
		int maior = arr[0].getId();
		for(int i=0; i<n; i++){ // obter o maior
			int x = arr[i].getId();
			if (x > maior) maior = x;
		}
		...
		for(int i=0; i<n; count[arr[i].getId()]++, i++);

		for(int i=1; i<count.length; count[i] += count[i-1], i++);

		for(int i=n-1; i>=0; i--){
			ordenado[count[arr[i].getId()]-1] = arr[i];
			count[arr[i].getId()]--;
			tp.move();
		}
	...
	static void sortRate(Pokemon[] arr, TP tp){
		...
		for(int i=n-1; i>=0; i--){
			ordenado[count[arr[i].getCaptureRate()]-1] = arr[i];
			count[arr[i].getCaptureRate()]--;
			tp.move();
		}
	...
	public static void main(String[] args){
		try{
			...
			tp.print("855947_countingsort.txt");
